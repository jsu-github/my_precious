...
export async function up(knex: Knex): Promise<void> {
  await knex.transaction(async trx => {
    ...
    const existing = await trx('tier_config').select('*').orderBy('tier_id');

    // 2. Drop and recreate with entity_scope column + composite PK
    await trx.schema.dropTableIfExists('tier_config');
    await trx.schema.createTable('tier_config', (table) => {
    ...
    await trx('tier_config').insert(rows);
  });
}
